...
public class RabbitMQConfig {
    ...
    //Exchange와 Queue 바인딩
    @Bean
    public Binding sendMessageBinding(Queue chatQueue, TopicExchange chatExchange) {
        return BindingBuilder.bind(chatQueue).to(chatExchange).with(CHAT_ROUTING_KEY);
    }

    @Bean
    public Binding chatRecentMessageBinding(Queue recentMessageQueue, TopicExchange chatExchange) {
        return BindingBuilder.bind(recentMessageQueue).to(chatExchange).with(CHAT_RECENT_MESSAGE_ROUTING_KEY);
    }

    @Bean
    public Binding chatConnectBinding(Queue chatConnectQueue, TopicExchange chatExchange) {
        return BindingBuilder.bind(chatConnectQueue).to(chatExchange).with(CHAT_CONNECT_ROUTING_KEY);
    }

    @Bean
    public Binding chatDisconnectBinding(Queue chatDisconnectQueue, TopicExchange chatExchange) {
        return BindingBuilder.bind(chatDisconnectQueue).to(chatExchange).with(CHAT_DISCONNECT_ROUTING_KEY);
    }
